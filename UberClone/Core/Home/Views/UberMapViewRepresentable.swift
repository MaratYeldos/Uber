...
struct UberMapViewRepresentable: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UIViewType, context: Context) {
        if let coordinate = locationViewModel.selectedLocationCoordinate {
            ...
            context.coordinator.configurePoliline(with: coordinate)
        }
    }
...
extension UberMapViewRepresentable {
    ...
    class MapCoordinator: NSObject, MKMapViewDelegate {
        ...
        func configurePoliline(with destionationCoordinate: CLLocationCoordinate2D) {
            guard let userLocationCoordinate = self.userLocationCoordinate else { return }
            getDestinationRoute(from: userLocationCoordinate, to: destionationCoordinate) { route in
                self.parent.mapView.addOverlay(route.polyline)
            }
        }
        
        func getDestinationRoute(from userLocation: CLLocationCoordinate2D, to destinationCoordine: CLLocationCoordinate2D, completion: @escaping (MKRoute) -> Void) {
